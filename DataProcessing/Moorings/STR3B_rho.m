...
%% Download Data 
B=load('/Volumes/InnerShelf1/Moorings/NPS_SIO_Moorings/nearshore/ptsal_tchain_STR3_B.mat');
B.TCHAIN.dn=B.TCHAIN.time_dnum+(7/24);
B.TCHAIN.zbedjm=B.TCHAIN.ZBEDT;
B.TCHAIN.zbedjm(1)=B.TCHAIN.EXPECTED_DEPTH_MSL;

% get rid of missing instruments
B.TCHAIN.tempi=B.TCHAIN.TEMP;
B.TCHAIN.tempi(:,3:4)=[];
B.TCHAIN.zbedjm(3:4)=[];
B.dn=B.TCHAIN.dn(1):((5/60)/24):B.TCHAIN.dn(end);
B.low=pl66tn(B.TCHAIN.tempi,1/60,15/60); %seconds to hours 
B.temp=interp1(B.TCHAIN.dn,B.low,B.dn);

B.mab=B.TCHAIN.zbedjm;

%% Transform to uniform grid in z 
dn=datenum(2017,9,1,0,0,0):1/(24*60):datenum(2017,10,30,0,0,0);
mab=(0:0.25:11)'; 

Temp=interp2(B.dn,B.mab,B.temp',dn,mab);
...
figure; 
subplot(2,1,1);
pcolorjw(B.dn,B.mab,B.temp'); colorbar 
xaxis(dn(1),dn(end));
yaxis(mab(1),mab(end));
...
clearvars -except dn B mab temp sal rho H sig Tsig Rho_sig dn_sig anadir D

if 0
    save([anadir 'STR3B_TempRho_zandsig.mat']);
end 
