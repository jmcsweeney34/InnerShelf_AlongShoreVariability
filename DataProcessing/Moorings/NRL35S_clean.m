...
NRL35S=load('/Volumes/InnerShelf1/Moorings/NPS_SIO_Moorings/NRL35S/T.mat');

%% Fix NRL temp - 50S  

NRL35S.dn=datenum(2017,9,6):60/(24*60*60):datenum(2017,11,6);
NRL35S.mab=flipud(NRL35S.T.zab);

NRL35S.temp(1:length(NRL35S.mab),1:length(NRL35S.dn))=nan;
NRL35S.temp(1,:)=interp1(datenum(NRL35S.T.datenum.z6),NRL35S.T.temp.z6,NRL35S.dn);
NRL35S.temp(2,:)=interp1(datenum(NRL35S.T.datenum.z9),NRL35S.T.temp.z9,NRL35S.dn);
NRL35S.temp(3,:)=interp1(datenum(NRL35S.T.datenum.z12),NRL35S.T.temp.z12,NRL35S.dn);
NRL35S.temp(4,:)=interp1(datenum(NRL35S.T.datenum.z15),NRL35S.T.temp.z15,NRL35S.dn);
NRL35S.temp(5,:)=interp1(datenum(NRL35S.T.datenum.z18),NRL35S.T.temp.z18,NRL35S.dn);
NRL35S.temp(6,:)=interp1(datenum(NRL35S.T.datenum.z21),NRL35S.T.temp.z21,NRL35S.dn);
NRL35S.temp(7,:)=interp1(datenum(NRL35S.T.datenum.z24),NRL35S.T.temp.z24,NRL35S.dn);
NRL35S.temp(8,:)=interp1(datenum(NRL35S.T.datenum.z27),NRL35S.T.temp.z27,NRL35S.dn);

xtemp=datenum(NRL35S.T.datenum.z30); ytemp=NRL35S.T.temp.z30;
[x, index] = unique(xtemp); 
NRL35S.temp(9,:)=interp1(x,ytemp(index),NRL35S.dn);

% % clever way to identify good data; but want bad data haha 
% i=(NRL35S.dn>datenum(2017,9,9,18,1,0)).*(NRL35S.dn<datenum(2017,10,31,3,0,0));
% i(i==0)=nan;

i=find(NRL35S.dn<datenum(2017,9,8,15,50,0) | NRL35S.dn>datenum(2017,10,29,16,50,0));
NRL35S.dn(i)=[];
NRL35S.temp(:,i)=[];
...
dn=datenum(2017,9,8,0,0,0):1/(24*60):datenum(2017,10,30,0,0,0);
mab=(0:0.5:36)'; 

t=interp2(NRL35S.dn,NRL35S.mab,NRL35S.temp,dn,mab);
...
D=interp1(tide.dn,tide.wl+35,dn);
D=pl33tn(D,1/60,2)';
...
%% 
if 0
    save([anadir 'NRL35S.mat'],'Tsig','dn_sig','H',...
        't','dn','mab','subtidal','-v7.3');
    disp('NRL35S.mat saved!');
end
