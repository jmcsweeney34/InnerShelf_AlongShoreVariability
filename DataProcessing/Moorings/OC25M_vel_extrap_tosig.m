...
%% Download Data 
oc25m=load('/Volumes/InnerShelf1/Moorings/NPS_SIO_Moorings/OC25M-A/enu-30secavg_OC25M-deployment1.mat');
oc25m_b=load('/Volumes/InnerShelf1/Moorings/NPS_SIO_Moorings/OC25M-A/OC25M_1min_JMM.mat');

%% Mask bad data (there seem to be a lot ot bad data at the surface)
%  being conservative here 

perbad=40;
oc25m.Z=meshgrid(oc25m.z,oc25m.t)';
oc25m.maskjm=double(oc25m.Z<(25-(25*perbad/100)));
oc25m.maskjm(oc25m.maskjm==0)=nan;

oc25m_b.Z=meshgrid(oc25m_b.mab,oc25m_b.dn)';
oc25m_b.maskjm=double(oc25m_b.Z<(25-(25*perbad/100)));
oc25m_b.maskjm(oc25m_b.maskjm==0)=nan;
...
u1=interp2(oc25m.t,double(oc25m.z),oc25m.u.*oc25m.maskjm,dn,mab);
u2=interp2(oc25m_b.dn,oc25m_b.mab,oc25m_b.u.*oc25m_b.maskjm,dn,mab);
u=u1.*nan;
i=~isnan(u1); u(i)=u1(i);
i=~isnan(u2); u(i)=u2(i);

v1=interp2(oc25m.t,double(oc25m.z),oc25m.v.*oc25m.maskjm,dn,mab);
v2=interp2(oc25m_b.dn,oc25m_b.mab,oc25m_b.v.*oc25m_b.maskjm,dn,mab);
v=v1.*nan;
i=~isnan(v1); v(i)=v1(i);
i=~isnan(v2); v(i)=v2(i);

w1=interp2(oc25m.t,double(oc25m.z),oc25m.w.*oc25m.maskjm,dn,mab);
w2=interp2(oc25m_b.dn,oc25m_b.mab,oc25m_b.w.*oc25m_b.maskjm,dn,mab);
w=w1.*nan;
i=~isnan(w1); w(i)=w1(i);
i=~isnan(w2); w(i)=w2(i);
...
if 1
    i=50013;
    ii=max((findnearest_JACK(oc25m_b.dn,dn(i))));
    
    close all
    figure('position',[440   116   325   682]);
    plot(oc25m_b.dn(:,ii),oc25m_b.mab,'.','color',[0.8 0.8 0.8],'markersize',40);
    hold on
...
dn1=datenum(2017,9,22,0,0,00);
dn2=datenum(2017,9,22,12,0,00);
...
clearvars -except dn mab EAST NORTH VERT oc25m oc25m_b anadir
...
hold on
try
    ii=max((findnearest_JACK(oc25sa.datenum,dn(i))));
...
for i=1:length(mab) 
    EAST(i,nn1)=fillmissing(EAST(i,nn1),'linear');
    EAST(i,nn2)=fillmissing(EAST(i,nn2),'linear');
end 

for i=1:length(sig) 
    Usig(i,nn1)=fillmissing(Usig(i,nn1),'linear');
    Usig(i,nn2)=fillmissing(Usig(i,nn2),'linear');
end 
%% Clear variables except those needed 
...
if 0
    save([anadir 'oc25m_vel_zandsig.mat']);
end 
