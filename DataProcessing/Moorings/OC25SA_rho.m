...
%% Download Data 
%%%% OC25SA-T is a messy product - let's fix it up a bit 
tempo=load('/Volumes/InnerShelf1/Moorings/NPS_SIO_Moorings/OC25SA-T/OC25SA-T_deploy2_30sec.mat');
% bad lat lon  
% OC25SA.lon=-tempo.grid30s.lon;
% OC25SA.lat=tempo.grid30s.lat;
OC25SA.lat= 34 + (58.531/60);  
OC25SA.lon=-(120 +(40.529/60) );
OC25SA.temp=tempo.grid30s.temp;
OC25SA.mab=25-tempo.grid30s.pos;
OC25SA.dn=tempo.grid30s.datenum;

ii=[1 2 3 4 5 10 12 18 20 22 ];
OC25SA.temp(ii,:)=[]; OC25SA.mab(ii)=[];
clear tempo
...
Temp=interp2(OC25SA.dn,OC25SA.mab,fillmissing(OC25SA.temp,'linear'),dn,mab);
...
figure; 
subplot(2,1,1);
pcolorjw(OC25SA.dn,OC25SA.mab,OC25SA.temp); colorbar 
hold on 
xaxis(dn(1),dn(end));
yaxis(mab(1),mab(end));
...
clearvars -except dn mab OC25SA temp anadir veldir
...
clearvars -except dn OC25SA  mab temp sal rho H sig Tsig Rho_sig dn_sig anadir D

if 0
    save([anadir 'OC25SA_TempRho_zandsig.mat']);
end 
