...
STR4F=load('/Volumes/InnerShelf1/Moorings/NPS_SIO_Moorings/STR4F/ptsal_tchain_STR4_F.mat');

STR4F.mab=STR4F.TCHAIN.ZBEDT;
STR4F.mab(1)=STR4F.mab(2)+1.15;

%% Need to convert from local time to UTC!! 
% added this line 9/19/19 after disovering the times didn't make sense.
% whoops! 
STR4F.TCHAIN.time_dnum=STR4F.TCHAIN.time_dnum+(7/24); 

%% %% Transform to uniform grid in z 
% pcolorjw(STR4F.TCHAIN.time_dnum,STR4F.mab,STR4F.TCHAIN.TEMP')

dn=datenum(2017,9,1,0,0,0):1/(24*60):datenum(2017,10,15,0,0,0);
mab=(0:0.25:10)'; 

% there was surface data that wasn't flagged appropriately 
t=interp2(STR4F.TCHAIN.time_dnum,STR4F.mab,STR4F.TCHAIN.TEMP',dn,mab);
...
if 1
    ...
    '/Volumes/InnerShelf1/JackAnalysis/AlongShelfVariability/CleanUpMoorings/STR4F_clean' ];

    save([anadir 'STR4F.mat'],'Tsig','dn_sig','H',...
        't','dn','sig','mab','subtidal','Psig','Rho_sig','-v7.3');
    disp('STR4F.mat saved!');
end
