...
NRL20N=load('/Volumes/InnerShelf1/Moorings/NPS_SIO_Moorings/NRL20N/T.mat');
nrl20n=load('/Volumes/InnerShelf1/Moorings/NPS_SIO_Moorings/NRL20N/NRL20N_AQ.mat');

%% Fix NRL temp

NRL20N.dn=datenum(2017,9,6):60/(24*60*60):datenum(2017,11,2);
NRL20N.mab=flipud(NRL20N.T.zab);

NRL20N.temp(1:length(NRL20N.mab),1:length(NRL20N.dn))=nan;
NRL20N.temp(1,:)=interp1(datenum(NRL20N.T.datenum.z3),NRL20N.T.temp.z3,NRL20N.dn);

xtemp=datenum(NRL20N.T.datenum.z6); ytemp=NRL20N.T.temp.z6;
[x, index] = unique(xtemp); 
NRL20N.temp(2,:)=interp1(x,ytemp(index),NRL20N.dn);

NRL20N.temp(3,:)=interp1(datenum(NRL20N.T.datenum.z9),NRL20N.T.temp.z9,NRL20N.dn);
NRL20N.temp(4,:)=interp1(datenum(NRL20N.T.datenum.z12),NRL20N.T.temp.z12,NRL20N.dn);
xtemp=datenum(NRL20N.T.datenum.z20); ytemp=NRL20N.T.temp.z20;
[x, index] = unique(xtemp); 
NRL20N.temp(5,:)=interp1(x,ytemp(index),NRL20N.dn);

% % clever way to identify good data; but want bad data haha 
% i=(NRL20N.dn>datenum(2017,9,9,18,1,0)).*(NRL20N.dn<datenum(2017,10,31,3,0,0));
% i(i==0)=nan;

i=find(NRL20N.dn<datenum(2017,9,9,16,55,0) | NRL20N.dn>datenum(2017,10,31,16,50,0));
...
dn=datenum(2017,9,8,0,0,0):1/(24*60):datenum(2017,11,2,0,0,0);
mab=(0:0.5:21)'; 

u=interp2(nrl20n.AQ.time_dnum,nrl20n.AQ.Zbed,nrl20n.AQ.Ue',dn,mab);
v=interp2(nrl20n.AQ.time_dnum,nrl20n.AQ.Zbed,nrl20n.AQ.Un',dn,mab);
t=interp2(NRL20N.dn,NRL20N.mab,NRL20N.temp,dn,mab);
...
%% 
if 0
    save([anadir 'NRL20N.mat'],'Usig','Vsig','Tsig','dn_sig','H',...
        'u','v','t','dn','mab','subtidal','-v7.3');
    disp('NRL20N.mat saved!');
end
