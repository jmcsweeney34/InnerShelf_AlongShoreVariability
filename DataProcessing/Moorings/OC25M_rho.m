...
%% Download Data 
%%%% OC25M-T is a messy product - let's fix it up a bit 
tempo=load('/Volumes/InnerShelf1/Moorings/NPS_SIO_Moorings/OC25M-T/OC25M-T_deploy2_30sec.mat');
OC25M.temp=tempo.grid30s.temp;
OC25M.mab=25-tempo.grid30s.pos;
OC25M.dn=tempo.grid30s.datenum;
OC25M.lat= 34 + (59.874/60);  
OC25M.lon= -(120 +(40.182/60) );

ii=[3 4 10 12 18 20 22];
OC25M.temp(ii,:)=[]; OC25M.mab(ii)=[];
clear tempo
...
Temp=interp2(OC25M.dn,OC25M.mab,fillmissing(OC25M.temp,'linear'),dn,mab);
...
figure; 
subplot(2,1,1);
pcolorjw(OC25M.dn,OC25M.mab,OC25M.temp); colorbar 
hold on 
xaxis(dn(1),dn(end));
yaxis(mab(1),mab(end));
...
clearvars -except dn mab OC25M temp anadir veldir
...
clearvars -except dn OC25M  mab temp sal rho H sig Tsig Rho_sig dn_sig anadir D

if 0
    save([anadir 'OC25M_TempRho_zandsig.mat']);
end 
