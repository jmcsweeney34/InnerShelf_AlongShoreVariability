...
PS40M=load('/Volumes/InnerShelf1/Moorings/NPS_SIO_Moorings/PS40M/PS40M-T_30s.mat');
ps40m=load('/Volumes/InnerShelf1/Moorings/NPS_SIO_Moorings/PS40M/PS40M-A_20s.mat');
...
ps40m.dn=datenum(2017,9,9,0,0,0):60/(60*60*24):datenum(2017,11,3,0,0,0);
ps40m.mab=ps40m.zab;

% a mistake on Colosi's part - an easy fix though 
ps40m.u=ps40m.u(1:17,:);
ps40m.v=ps40m.v(1:17,:);

% Make new mask - the other one kept too much bad data  
perbad=10;
ps40m.z=meshgrid(ps40m.zab,ps40m.datenum)';
ps40m.maskjm=double(ps40m.z<(40-(40*perbad/100)));
ps40m.maskjm(ps40m.maskjm==0)=nan;

tempo=pl66tn(ps40m.u.*ps40m.maskjm,20/3600,60/3600)'; %1 min filter, 20 second data 
ps40m.ui=interp2(meshgrid(ps40m.datenum,ps40m.mab),meshgrid(ps40m.mab,ps40m.datenum)',tempo,meshgrid(ps40m.dn,ps40m.mab),meshgrid(ps40m.mab,ps40m.dn)');
clear tempo 
tempo=pl66tn(ps40m.v.*ps40m.maskjm,20/3600,60/3600)'; %1 min filter, 20 second data 
ps40m.vi=interp2(meshgrid(ps40m.datenum,ps40m.mab),meshgrid(ps40m.mab,ps40m.datenum)',tempo,meshgrid(ps40m.dn,ps40m.mab),meshgrid(ps40m.mab,ps40m.dn)');
clear tempo 
tempo=pl66tn(ps40m.w.*ps40m.maskjm,20/3600,60/3600)'; %1 min filter, 20 second data 
ps40m.wi=interp2(meshgrid(ps40m.datenum,ps40m.mab),meshgrid(ps40m.mab,ps40m.datenum)',tempo,meshgrid(ps40m.dn,ps40m.mab),meshgrid(ps40m.mab,ps40m.dn)');
clear tempo 


% pcolorjw(repmat(PS40M.datenum,9,1)',-PS40M.pres+40,PS40M.temp); colorbar
...
u=interp2(ps40m.dn,ps40m.mab,ps40m.ui,dn,mab);
v=interp2(ps40m.dn,ps40m.mab,ps40m.vi,dn,mab);
w=interp2(ps40m.dn,ps40m.mab,ps40m.wi,dn,mab);

%griddata was acting weird - this is slow but gets the job done 
t=u.*nan; 
for i=1:length(dn)
    ii=findnearest_JACK(PS40M.datenum,dn(i));
    if abs(dn(i)-PS40M.datenum(ii)) < 0.000001
        t(:,i)=interp1(-PS40M.pres(ii,:)+40,PS40M.temp(ii,:),mab);
    end 
end
...
%%
if 0
    save([anadir 'PS40M.mat'],'Usig','Vsig','Wsig','Tsig','dn_sig','H',...
        'u','v','w','t','dn','mab','subtidal','-v7.3');
    disp('PS40M.mat saved!');
end
