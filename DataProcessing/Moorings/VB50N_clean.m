...
VB50N=load('/Volumes/InnerShelf1/Moorings/NPS_SIO_Moorings/VB50N/VB50N-T_30s.mat');
VB50N.dn=repmat(VB50N.datenum,9,1)';
% vb50N=load('/Volumes/InnerShelf1/Moorings/NPS_SIO_Moorings/VB50N/VB50N-A_20s.mat');
% % level 2 data for PS50 had inncorrect dimensions for u and v; sent John
% an email but working with level 1 data from here.  
vb50N=load('/Volumes/InnerShelf1/Moorings/NPS_SIO_Moorings/VB50N/SN6958.mat');


%% Clean data

% Make mask for vb50n - the other one kept too much bad data  
perbad=10;
vb50N.A.z=meshgrid(vb50N.A.zab,vb50N.A.datenum)';
vb50N.A.maskjm=double(vb50N.A.z<(50-(50*perbad/100)));
vb50N.A.maskjm(vb50N.A.maskjm==0)=nan;

vb50N.dn=datenum(2017,9,10,16,0,0):60/(60*60*24):datenum(2017,10,30,2,0,0);
vb50N.mab=vb50N.A.zab;
tempo=pl66tn(vb50N.A.u.*vb50N.A.maskjm,4/3600,60/3600)'; %1 min filter, 4 second data 
vb50N.u=interp2(meshgrid(vb50N.A.datenum,vb50N.mab),meshgrid(vb50N.mab,vb50N.A.datenum)',tempo,meshgrid(vb50N.dn,vb50N.mab),meshgrid(vb50N.mab,vb50N.dn)');
clear tempo 
tempo=pl66tn(vb50N.A.v.*vb50N.A.maskjm,4/3600,60/3600)'; %1 min filter, 4 second data 
vb50N.v=interp2(meshgrid(vb50N.A.datenum,vb50N.mab),meshgrid(vb50N.mab,vb50N.A.datenum)',tempo,meshgrid(vb50N.dn,vb50N.mab),meshgrid(vb50N.mab,vb50N.dn)');
clear tempo 
tempo=pl66tn(vb50N.A.w.*vb50N.A.maskjm,4/3600,60/3600)'; %1 min filter, 4 second data 
vb50N.w=interp2(meshgrid(vb50N.A.datenum,vb50N.mab),meshgrid(vb50N.mab,vb50N.A.datenum)',tempo,meshgrid(vb50N.dn,vb50N.mab),meshgrid(vb50N.mab,vb50N.dn)');
clear tempo
...
u=interp2(vb50N.dn,vb50N.mab,vb50N.u,dn,mab);
v=interp2(vb50N.dn,vb50N.mab,vb50N.v,dn,mab);
w=interp2(vb50N.dn,vb50N.mab,vb50N.w,dn,mab);

%griddata was acting weird - this is slow but gets the job done 
t=u.*nan; 
for i=1:length(dn)
    ii=findnearest_JACK(VB50N.datenum,dn(i));
    if abs(dn(i)-VB50N.dn(ii)) < 0.000001
        t(:,i)=interp1(-VB50N.pres(ii,:)+51,VB50N.temp(ii,:),mab);
    end 
end
    ...
    if isnan(ttmp(ks))
        if ~isnan(nanmean(ttmp))
            try
                ...
                ttmpi=fillmissing(ttmp(ks-5:ks+2),'linear');
                [xx,~]=polyfit(ttmpi,ztmp(ks-5:ks+2),2);
            end
...
%% 
if 1
    save([anadir 'VB50N.mat'],'Usig','Vsig','Wsig','Tsig','Rho_sig','Psig',...
        'dn_sig','sig','H','u','v','w','t','dn','mab','subtidal','-v7.3');
    disp('VB50N.mat saved!');    
end
