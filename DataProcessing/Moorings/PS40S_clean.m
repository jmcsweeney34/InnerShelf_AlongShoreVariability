...
PS40S=load('/Volumes/InnerShelf1/Moorings/NPS_SIO_Moorings/PS40S/PS40S-T_30s.mat');
ps40s=load('/Volumes/InnerShelf1/Moorings/NPS_SIO_Moorings/PS40S/PS40S-A_20s.mat');
...
ps40s.dn=datenum(2017,9,10,0,0,0):60/(60*60*24):datenum(2017,11,2,0,0,0);
ps40s.mab=ps40s.zab;

% a mistake on Colosi's part - an easy fix though 
ps40s.u=ps40s.u(1:17,:);
ps40s.v=ps40s.v(1:17,:);

% Make new mask - the other one kept too much bad data  
perbad=10;
ps40s.z=meshgrid(ps40s.zab,ps40s.datenum)';
ps40s.maskjm=double(ps40s.z<(40-(40*perbad/100)));
ps40s.maskjm(ps40s.maskjm==0)=nan;

tempo=pl66tn(ps40s.u.*ps40s.maskjm,20/3600,60/3600)'; %1 min filter, 20 second data 
ps40s.ui=interp2(meshgrid(ps40s.datenum,ps40s.mab),meshgrid(ps40s.mab,ps40s.datenum)',tempo,meshgrid(ps40s.dn,ps40s.mab),meshgrid(ps40s.mab,ps40s.dn)');
clear tempo 
tempo=pl66tn(ps40s.v.*ps40s.maskjm,20/3600,60/3600)'; %1 min filter, 20 second data 
ps40s.vi=interp2(meshgrid(ps40s.datenum,ps40s.mab),meshgrid(ps40s.mab,ps40s.datenum)',tempo,meshgrid(ps40s.dn,ps40s.mab),meshgrid(ps40s.mab,ps40s.dn)');
clear tempo 
tempo=pl66tn(ps40s.w.*ps40s.maskjm,20/3600,60/3600)'; %1 min filter, 20 second data 
ps40s.wi=interp2(meshgrid(ps40s.datenum,ps40s.mab),meshgrid(ps40s.mab,ps40s.datenum)',tempo,meshgrid(ps40s.dn,ps40s.mab),meshgrid(ps40s.mab,ps40s.dn)');
clear tempo 


% pcolorjw(repmat(PS40S.datenum,9,1)',-PS40S.pres+40,PS40S.temp); colorbar

%% %% Transform to uniform grid in z 

dn=datenum(2017,9,10,0,0,0):1/(24*60):datenum(2017,11,2,0,0,0);
mab=(0:1:41)'; 

u=interp2(ps40s.dn,ps40s.mab,ps40s.ui,dn,mab);
v=interp2(ps40s.dn,ps40s.mab,ps40s.vi,dn,mab);
w=interp2(ps40s.dn,ps40s.mab,ps40s.wi,dn,mab);

%griddata was acting weird - this is slow but gets the job done 
t=u.*nan; 
for i=1:length(dn)
    ii=findnearest_JACK(PS40S.datenum,dn(i));
    if abs(dn(i)-PS40S.datenum(ii)) < 0.000001
        t(:,i)=interp1(-PS40S.pres(ii,:)+40,PS40S.temp(ii,:),mab);
    end 
end
...
D=interp1(tide.dn,tide.wl+40,dn);
D=pl33tn(D,1/60,2)';
...
if 0
    save([anadir 'PS40S.mat'],'Usig','Vsig','Wsig','Tsig','dn_sig','H',...
        'u','v','w','t','dn','mab','subtidal','-v7.3');
    disp('PS40S.mat saved!');
end
