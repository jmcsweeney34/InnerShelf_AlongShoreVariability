...
NRL20S=load('/Volumes/InnerShelf1/Moorings/NPS_SIO_Moorings/NRL20S/T.mat');
nrl20s=load('/Volumes/InnerShelf1/Moorings/NPS_SIO_Moorings/NRL20S/NRL20S_1min.mat');

%% Fix NRL temp 

NRL20S.dn=datenum(2017,9,6):60/(24*60*60):datenum(2017,11,6);
NRL20S.mab=flipud(NRL20S.T.zab);

NRL20S.temp(1:length(NRL20S.mab),1:length(NRL20S.dn))=nan;
NRL20S.temp(1,:)=interp1(datenum(NRL20S.T.datenum.z3),NRL20S.T.temp.z3,NRL20S.dn);
NRL20S.temp(2,:)=interp1(datenum(NRL20S.T.datenum.z6),NRL20S.T.temp.z6,NRL20S.dn);
NRL20S.temp(3,:)=interp1(datenum(NRL20S.T.datenum.z9),NRL20S.T.temp.z9,NRL20S.dn);
NRL20S.temp(4,:)=interp1(datenum(NRL20S.T.datenum.z12),NRL20S.T.temp.z12,NRL20S.dn);
NRL20S.temp(5,:)=interp1(datenum(NRL20S.T.datenum.z20),NRL20S.T.temp.z20,NRL20S.dn);
...
i=find(NRL20S.dn<datenum(2017,9,10,19,21,0) | NRL20S.dn>datenum(2017,10,29,17,30,0));
NRL20S.dn(i)=[];
NRL20S.temp(:,i)=[];

%% Clean up the ADCP 

i=(nrl20s.A.datenum<datenum(2017,9,10,20,0,0));
...
u=interp2(nrl20s.A.datenum,nrl20s.A.zab,nrl20s.A.u,dn,mab);
v=interp2(nrl20s.A.datenum,nrl20s.A.zab,nrl20s.A.v,dn,mab);
w=interp2(nrl20s.A.datenum,nrl20s.A.zab,nrl20s.A.w5,dn,mab);
t=interp2(NRL20S.dn,NRL20S.mab,NRL20S.temp,dn,mab);
...
D=interp1(tide.dn,tide.wl+20,dn);
D=pl33tn(D,1/60,2)';
...
for i =5482:length(dn)
    ...
    if length(ttmp(~isnan(ttmp)))>5        
        kb=find(~isnan(ttmp),1);
        ttmpi=fillmissing(ttmp(kb:kb+3),'linear');
        [xx, ~]=polyfit(ttmpi,ztmp(kb:kb+3),1);
...
%% 
if 0
    save([anadir 'NRL20S.mat'],'Usig','Vsig','Wsig','Tsig','dn_sig','H',...
        'u','v','w','t','dn','mab','subtidal','-v7.3');    
    disp('NRL20S.mat saved!');
end
