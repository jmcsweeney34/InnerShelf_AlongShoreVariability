...
NW2=load('/Volumes/InnerShelf1/Moorings/NPS_SIO_Moorings/NW2/ptsal_tchain_NW2.mat');
nw2=load('/Volumes/InnerShelf1/Moorings/NPS_SIO_Moorings/NW2/NW2A_AQ.mat');

%% Need to convert from local time to UTC!! 
% added this line 9/19/19 after disovering the times didn't make sense.
% whoops! 

NW2.TCHAIN.time_dnum30=NW2.TCHAIN.time_dnum30+(7/24); 
NW2.TCHAIN.time_dnum60=NW2.TCHAIN.time_dnum60+(7/24); 

nw2.AQ.time_dnum=nw2.AQ.time_dnum+(7/24); 

%% Fix temperature 

NW2.mab=NW2.TCHAIN.ZBEDT;
NW2.mab(1)=NW2.mab(2)+1.5;

% ii=find(isnan(NW2.TCHAIN.TEMP30(:,4)));
NW2.temp=NW2.TCHAIN.TEMP30;

for i=81120:length(NW2.TCHAIN.time_dnum30)
    NW2.temp(i,:)=fillmissing(NW2.temp(i,:),'linear'); 
end 

%% %% Transform to uniform grid in z 
% pcolorjw(NW2.TCHAIN.time_dnum30,NW2.TCHAIN.ZBEDT,NW2.TCHAIN.TEMP30')
% pcolorjw(NW2.TCHAIN.time_dnum30,NW2.mab,NW2.TCHAIN.TEMP30')
% pcolorjw(NW2.TCHAIN.time_dnum30,NW2.mab,NW2.temp')

dn=datenum(2017,9,1,0,0,0):1/(24*60):datenum(2017,10,16,0,0,0);
mab=(0:0.5:13)'; 

% some bad data at the bed that wasn't flagged correctly 
u=interp2(nw2.AQ.time_dnum,nw2.AQ.Zbed(4:end),nw2.AQ.Ue(:,4:end)',dn,mab);
v=interp2(nw2.AQ.time_dnum,nw2.AQ.Zbed(4:end),nw2.AQ.Un(:,4:end)',dn,mab);
t=interp2(NW2.TCHAIN.time_dnum30,NW2.mab,NW2.temp',dn,mab);
...
D=interp1(tide.dn,tide.wl+12,dn);
D=pl33tn(D,1/60,2)';
...
if 0
    ...
    '/Volumes/InnerShelf1/JackAnalysis/AlongShelfVariability/CleanUpMoorings/NW2_clean' ];

    save([anadir 'NW2.mat'],'Usig','Vsig','Tsig','dn_sig','H',...
        'u','v','t','dn','sig','mab','subtidal','info','Psig','Rho_sig','-v7.3');
    disp('NW2.mat saved!');
end
