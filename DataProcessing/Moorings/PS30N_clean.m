...
PS30N=load('/Volumes/InnerShelf1/Moorings/NPS_SIO_Moorings/PS30N/PS30N-T_30s.mat');


%% %% Transform to uniform grid in z 

dn=datenum(2017,9,9,0,0,0):1/(24*60):datenum(2017,11,2,0,0,0);
mab=(0:1:31)'; 

%griddata was acting weird - this is slow but gets the job done 
t(1:length(mab),1:length(dn))=nan; 
for i=1:length(dn)
    ii=findnearest_JACK(PS30N.datenum,dn(i));
    if abs(dn(i)-PS30N.datenum(ii)) < 0.000001
        t(:,i)=interp1(-PS30N.pres(ii,:)+28,PS30N.temp(ii,:),mab);
    end 
end
...
%% 
if 1
    save([anadir 'PS30N.mat'],'Tsig','dn_sig','H',...
        't','dn','mab','subtidal','-v7.3');
    disp('PS30N.mat saved!');
end
