...
PS30M=load('/Volumes/InnerShelf1/Moorings/NPS_SIO_Moorings/PS30M/PS30M-T_30s.mat');
ps30m=load('/Volumes/InnerShelf1/Moorings/NPS_SIO_Moorings/PS30M/PS30M-A_20s.mat');
...
ps30m.dn=datenum(2017,9,9,0,0,0):60/(60*60*24):datenum(2017,11,3,0,0,0);
ps30m.mab=ps30m.zab;

% a mistake on Colosi's part - an easy fix though 
ps30m.u=ps30m.u(1:22,:);
ps30m.v=ps30m.v(1:22,:);

% Make new mask - the other one kept too much bad data  
perbad=20;
ps30m.z=meshgrid(ps30m.zab,ps30m.datenum)';
ps30m.maskjm=double(ps30m.z<(30-(30*perbad/100)));
ps30m.maskjm(ps30m.maskjm==0)=nan;

tempo=pl66tn(ps30m.u.*ps30m.maskjm,20/3600,60/3600)'; %1 min filter, 20 second data 
ps30m.ui=interp2(meshgrid(ps30m.datenum,ps30m.mab),meshgrid(ps30m.mab,ps30m.datenum)',tempo,meshgrid(ps30m.dn,ps30m.mab),meshgrid(ps30m.mab,ps30m.dn)');
clear tempo 
tempo=pl66tn(ps30m.v.*ps30m.maskjm,20/3600,60/3600)'; %1 min filter, 20 second data 
ps30m.vi=interp2(meshgrid(ps30m.datenum,ps30m.mab),meshgrid(ps30m.mab,ps30m.datenum)',tempo,meshgrid(ps30m.dn,ps30m.mab),meshgrid(ps30m.mab,ps30m.dn)');
clear tempo 
tempo=pl66tn(ps30m.w.*ps30m.maskjm,20/3600,60/3600)'; %1 min filter, 20 second data 
ps30m.wi=interp2(meshgrid(ps30m.datenum,ps30m.mab),meshgrid(ps30m.mab,ps30m.datenum)',tempo,meshgrid(ps30m.dn,ps30m.mab),meshgrid(ps30m.mab,ps30m.dn)');
clear tempo 


% pcolorjw(repmat(PS30M.datenum,8,1)',-PS30M.pres+26,PS30M.temp); colorbar
...
u=interp2(ps30m.dn,ps30m.mab,ps30m.ui,dn,mab);
v=interp2(ps30m.dn,ps30m.mab,ps30m.vi,dn,mab);
w=interp2(ps30m.dn,ps30m.mab,ps30m.wi,dn,mab);

%griddata was acting weird - this is slow but gets the job done 
t=u.*nan; 
for i=1:length(dn)
    ii=findnearest_JACK(PS30M.datenum,dn(i));
    if abs(dn(i)-PS30M.datenum(ii)) < 0.000001
        t(:,i)=interp1(-PS30M.pres(ii,:)+26,PS30M.temp(ii,:),mab);
    end 
end
...
if 0
    save([anadir 'PS30M.mat'],'Usig','Vsig','Wsig','Tsig','dn_sig','H',...
        'u','v','w','t','dn','mab','subtidal','-v7.3');
    disp('PS30M.mat saved!');
end
