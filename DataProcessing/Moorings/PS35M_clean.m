...
PS35M=load('/Volumes/InnerShelf1/Moorings/NPS_SIO_Moorings/PS35M/PS35M-T_30s.mat');


%% %% Transform to uniform grid in z 

dn=datenum(2017,9,10,0,0,0):1/(24*60):datenum(2017,11,3,0,0,0);
mab=(0:0.5:36)'; 

t(1:length(mab),1:length(dn))=nan; 
for i=1:length(dn)
    ii=findnearest_JACK(PS35M.datenum,dn(i));
    if abs(dn(i)-PS35M.datenum(ii)) < 0.000001
        t(:,i)=interp1(-PS35M.pres(ii,:)+30,PS35M.temp(ii,:),mab);
    end 
end
...
%% 
if 0
    save([anadir 'PS35M.mat'],'Tsig','dn_sig','H',...
        't','dn','mab','subtidal','-v7.3');
    disp('PS35M.mat saved!');
end
